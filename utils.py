...
def update_group_product_quantity(parent_product_id: int, child_product_id: int, new_quantity: int) -> bool:
    ...
    # 🚨 Payload النظيف لتجنب خطأ 422 من سلة (لا نرسل quantity أو price إطلاقاً)
    payload = {
...
def remove_product_from_group(parent_product_id: int, child_product_id: int) -> bool:
    ...
    for item in parent.get('grouped_items', []):
        pid = item.get('product_id') or item.get('product', {}).get('id')
        if str(pid) != str(child_product_id):
            items.append({"product_id": int(pid), "quantity": int(item.get('quantity', 1))})
...
def add_product_to_group(parent_product_id: int, child_product_id: int) -> bool:
    ...
    for item in parent.get('grouped_items', []):
        pid = item.get('product_id') or item.get('product', {}).get('id')
        if str(pid) == str(child_product_id): return False
        items.append({"product_id": int(pid), "quantity": int(item.get('quantity', 1))})
        
    items.append({"product_id": int(child_product_id), "quantity": 1})
    payload = {
        "name": parent.get('name'),
        "type": "group_products",
        "grouped_items": items
...
# دوال مساعدة لإنشاء قوالب (فارغة لتقليل الكود هنا)
def fill_salla_template(products): return b""
def generate_salla_new_products_file(products): return b""
def export_products_to_excel(products): return b""
