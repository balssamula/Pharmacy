...
is_webhook = False
try:
    # استخدام getattr للتحقق بأمان من الخصائص دون التسبب في انهيار التطبيق
...
# إضافة لمسات CSS مخصصة لتنسيق الأزرار والكروت وصف العناوين
st.markdown("""
    <style>
...
st.sidebar.divider()
page = st.sidebar.radio("القائمة الرئيسية التطبيق:", [
...
if page == "🎁 إدارة العروض الجماعية (Excel Upload)":
    st.markdown("<h1 class='main-header'>🎁 التحكم الجماعي بالعروض الخاصة</h1>", unsafe_allow_whitespace=True)
    
    c1, c2 = st.columns([3, 1])
    with c1:
        st.markdown("<p style='font-size:16px;'>قم بتنزيل النموذج وتعبئة التواريخ مع الوقت بدقة، ثم أعد رفع الملف للتحديث الفوري.</p>", unsafe_allow_whitespace=True)
    with c2:
        st.download_button(
...
# --- الصفحة الثانية: جلب، تفعيل، إيقاف وحذف العروض حياً من التطبيق ---
    ...
    st.markdown("<h1 class='main-header'>📊 إدارة ومتابعة عروض المتجر حياً</h1>", unsafe_allow_whitespace=True)
        ...
        if res.status_code == 200:
            ...
            if not offers:
                ...
                for idx, offer in enumerate(offers):
                    ...
                    with col_act1:
                        # تبديل الحالة الفوري (Change Special Offer Status API)
                        ...
                        if st.button(btn_label, key=f"status_btn_{offer['id']}_{idx}"):
                            ...
                            st.rerun()
                    with col_act2:
                        # حذف العرض الفوري (Delete Special Offer API)
...
# --- 3. صفحة جلب ونسخ معرفات المنتجات لسهولة الاستخدام ---
    ...
    st.markdown("<h1 class='main-header'>📦 مستعرض المنتجات وجرد معرفات الـ ID</h1>", unsafe_allow_whitespace=True)
    ...
    if ACCESS_TOKEN:
        ...
        # جلب قائمة المنتجات الفورية من المتجر
